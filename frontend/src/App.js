...
export default function App() {
  ...
  async function reedemEth() {
    ...
    await signerContract
      .getEtherBack(ethers.utils.parseEther(reedem.toString()))
      .then((res) => console.log(res))
      .catch((error) => alert(error));
  }
  ...
  async function _transfer() {
    ...
    const result = await signerContract.transferBack(
      ethers.utils.parseEther(checkout[0]),
      address
    );
  }
  ...
  useEffect(async () => {
    try {
      ...
      const _checkout = await signerContract.getCheckout(
        await signer.getAddress()
      );
      this.setBalance(ethers.utils.formatEther(bal.toString()) + " " + "ETH");
    ...
    <div className="App">
      ...
      <div className="Connect">
        {userAddress == "" ? (
          ...
          <p style={{ color: "white" }}>Connected: {userAddress}</p>
        )}
      </div>
      <div className="TitleText">
        <h1>The Easiest Way to Borrow & Stream</h1>
      </div>
      <div className="Card">
